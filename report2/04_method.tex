...
\begin{enumerate}
    ...
    \item スイッチの電源を切り，スイッチから全ての配線を取り外す．コンソールケーブルをコンピュータのUSB端子に挿入し，\texttt{/dev/ttyUSB0}が存在するか確認したのち，以下のコマンドを実行する．
          \begin{lstlisting}
...
switch5(config)# no spanning-tree vlan 1
    \end{lstlisting}
    \item VLANを定義する．ここでは，1から8番ポートをバックボーン用，9から16番ポートを5C班用，17から24番ポートを5i班用のVLANと定義する．
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \node[port,fill=yellow!70!black](p00){};
        \node[port,below=.2cm of p00,fill=yellow!70!black](p10){};
        \foreach \u[evaluate=\u as \v using \u-1] in {1,2,...,11}{
                \node[fill=\ifnum\u<4yellow!70!black\else\ifnum\u<8magenta\else cyan\fi\fi ,port,right=.2cm of p0\v](p0\u){};
                \node[fill=\ifnum\u<4yellow!70!black\else\ifnum\u<8magenta\else cyan\fi\fi ,port,right=.2cm of p1\v](p1\u){};
            }
