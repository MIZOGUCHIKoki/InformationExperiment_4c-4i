...
\footnotetext[2]{本レポートで用いる図の一部は\cite{networkfig}の公開済図形を利用している．}
\section{名前解決フロー}
...
ここで，\texttt{username@domain}のユーザ名\texttt{username}宛に送りたいが，ユーザ名\texttt{username}を公開したくない場合，
\alias ファイルに\texttt{dummy}と\texttt{username}を対応づける設定をすることで，
\texttt{dummy@domain}でのメール受信が可能になる．
この機能はユーザ名を秘匿にしたい場合だけでなく，たとえば\texttt{usernameA}，\texttt{usernameB}，\texttt{usernameC}の3ユーザを1つのエイリアス\texttt{group}と対応づけて，
\texttt{group@domain}宛に送信したメールを3ユーザで受信できる．
